...
fn main() {
  ...
  let re_log_httpaccess:Regex = Regex::new("^.* https://[^ ]+ \"([A-Z]+) /subscriptions/([^/]+)/(?:(locations)|(?:resourceGroups/[^/]+/)?providers/([^/]+)/([^/]+)(?:/[^/]+/?([^/]+)?)*)\\?api-version=[0-9]{4}-[0-9]{2}-[0-9]{2} HTTP/1.1\" [0-9]{3} (?:[0-9]+|None)").unwrap();
  let re_log_additionalpermission:Regex = Regex::new("however, it does not have permission to perform action '([^']+)' on the linked scope\\(s\\) '/subscriptions/([^/]+)/(?:(locations)|(?:resourceGroups/[^/]+/)?providers/([^/]+)/([^/]+)(?:/[^/]+/?([^/]+)?)*)' or the linked scope\\(s\\) are invalid.").unwrap();

  let re_log_azcommand =
    Regex::new("^[\\s\\S]+ Command arguments: \\[((?:'[^-][^']+'(?:, )?)+)").unwrap();
  ...
  loop {
    ...
    if args.debug {
      match re_log_level.captures(input.as_str()) {
        Some(caps) => {
          eprint!(
            "{}{}\x1b[m",
            log_color_map[caps.get(1).unwrap().as_str()],
    ...
    match re_log_httpaccess.captures(input.as_str()) {
      Some(caps) => {
      ...
      None => match re_log_additionalpermission.captures(input.as_str()) {
        Some(caps) => {
          let verb = caps.get(1);
          let subscription_id = caps.get(2);
          let subscription_resource = caps.get(3);
        ...
        None => match re_log_azcommand.captures(input.as_str()) {
          Some(caps) => {
            let command = String::from(caps.get(1).unwrap().as_str()).replace("', ", "").replace("'", " ");
            *role_definition.get_mut("description").unwrap() = json!(format!("Custom role definition for `az{}` generated by custom-role-generator", command));
          }
          None => {}
        },
      },
