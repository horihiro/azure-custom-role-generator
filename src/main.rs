...
fn main() {
  ...
  loop {
    ...
    match re1.captures(input.as_str()) {
      Some(caps) => {
        ...
        action.push_str("/");
        action.push_str(method2action.get(&String::from(method.unwrap().as_str())).unwrap());
  ...
  *role_definition.get_mut("AssignableScopes").unwrap() = json!(vec_assignable_scopes);
  ...
  println!("{}", serde_json::to_string_pretty(&role_definition).unwrap());
}
